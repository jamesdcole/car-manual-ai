...
function MechanicSeatButton({ user }: { user: any }) {
  const [loading, setLoading] = useState(false);

  const upgradeToMechanicSeat = () => {  // ← NO 'async' needed
    if (!user) {
    ...
    setLoading(true);
    // Direct to Stripe Payment Link - works everywhere
    window.location.href = 'https://buy.stripe.com/test_5kQ14gcH6gGg4rVblW3oA00';
    // setLoading(false) not needed - page redirects instantly
  };
  ...
  return (
    <button
      ...
      className="group relative w-full bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white font-bold py-4 px-8 rounded-2xl text-xl shadow-2xl hover:shadow-orange-500/50 transform hover:-translate-y-1 transition-all duration-300 border-4 border-orange-400/50 hover:border-orange-300/50 disabled:opacity-50 disabled:cursor-not-allowed"
    >
      {loading ? 'Processing...' : user ? (
        <>
          🛠️ Mechanic Seat - $39/mo
          <span className="block text-lg font-normal mt-1 opacity-90">(Unlimited Rusty AI)</span>
        </>
      ) : (
        '🔒 Log in to Upgrade'
      )}
...
// 👇 BACK BUTTON - Add this right after your main component starts
const BackToHomeButton = () => {
  ...
  return (
    <button
      onClick={() => router.push('/')}
      className="fixed top-4 left-4 z-50 p-3 bg-slate-900/90 backdrop-blur-xl border border-slate-700 rounded-2xl text-slate-300 hover:bg-slate-800 hover:text-white transition-all duration-200 shadow-2xl hover:shadow-emerald-500/25 text-lg font-semibold flex items-center gap-2"
    >
      ← Back to Home
    </button>
  );
...
export default function Home() {
  // 🔥 SPLASH PAGE - Show before login
  const [showApp, setShowApp] = useState(false);
  
  // 🔐 Auth state (YOUR EXISTING CODE - UNCHANGED)
  const [user, setUser] = useState<null | { uid: string; email: string | null; emailVerified: boolean }>(null);
  ...
  // 👇 MOBILE PWA TOUCH FIX - Add this useEffect
useEffect(() => {
  if (typeof window !== 'undefined') {
    // Enable touch events for PWA
  ...
  await signOut(auth);  // ← Your existing Firebase signOut - PERFECT
};
  ...
  const handleAsk = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        'https://us-central1-car-manual-ai-5c053.cloudfunctions.net/ask',
        {
          method: 'POST',
...
if (authLoading) {
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      <div className="text-lg font-semibold text-gray-700">Checking login…</div>
    </div>
  );
}
...
if (!user) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-900">
      <div className="text-lg font-semibold text-white">Redirecting to login…</div>
    </div>
  );
}
...
if (!user.emailVerified) {
  return (
    <main className="min-h-screen flex items-center justify-center bg-slate-900">
      <div className="bg-white/90 p-8 rounded-3xl shadow-2xl w-full max-w-md text-center">
        ...
        <p className="mb-4 text-gray-700">
          ...
          Click the link in your inbox, then refresh your status.
        </p>
        <button
          onClick={async () => {
          ...
          className="w-full mb-3 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-semibold transition-colors"
        >
          🔄 I've clicked the link – refresh status
        </button>
        <button
          onClick={async () => {
            if (!auth.currentUser) return;
...
return (
  <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 py-8 px-4 sm:px-6 lg:px-8 relative">
    ...
    <div className="pt-20 lg:pt-24 max-w-4xl mx-auto">
      {/* HEADER */}
      <div className="text-center mb-8 md:mb-12 pt-4 md:pt-0">
        <div className="inline-flex items-center px-4 sm:px-6 py-3 rounded-full bg-white/95 backdrop-blur-xl shadow-2xl border border-white/60 mb-4 md:mb-6">
          ...
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-black bg-gradient-to-r from-emerald-500 via-orange-400 to-yellow-300 bg-clip-text text-transparent leading-tight">
            🛠️ GarageOEMai · Mechanic Seat
          </h1>
        </div>
        <p className="mt-2 text-xs sm:text-sm text-amber-500 bg-amber-50/80 border border-amber-200 px-3 py-1 rounded-full inline-flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse" />
          7 Day Free Trial · Sign Up anytime.
        </p>
        {/* 🔐 Signed in user */}
        <p className="text-sm text-gray-700 mb-2">
          ...
          <button
            onClick={handleSignOut}
            className="ml-2 text-blue-600 underline font-medium"
          >
            Sign out
          </button>
        </p>
        <p className="text-base sm:text-lg md:text-xl text-gray-800 max-w-2xl mx-auto leading-relaxed font-medium">
          Upload your vehicle's OEM manual. Ask anything. Get expert answers powered by AI (Rusty).
        </p>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white/90 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-6 shadow-2xl border border-white/50 hover:shadow-3xl transition-all duration-500">
                  ...
                  Save up to three manuals you use the most. Click a box to switch vehicles.
                  </p>
                ...
                {/* 🔥 MECHANIC SEAT - NEW ADDITION HERE */}
                {user && (
                <div className="p-4 bg-gradient-to-b from-orange-500 to-red-500 rounded-2xl shadow-2xl border-4 border-orange-400/30">
                <h3 className="text-xl font-black text-white mb-2 flex items-center">
                🛠️ Mechanic Seat
                </h3>
                <p className="text-orange-100 text-sm mb-4">
                $39/mo – Unlimited manuals + AI
                </p>
                <MechanicSeatButton user={user} />
                </div>
              )}
                ...
                <div className="bg-white/90 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-6 sm:p-8 shadow-2xl border border-white/50 hover:shadow-3xl transition-all duration-500">
                  ...
                  >
                    {isAsking ? (
                     ...
                     <>
                        🚀 Ask Manual
                        <span className="sm:inline hidden ml-2">Expert</span>
                    </>
                    )}
                  </button>
                </div>
...
<button
  onClick={() => {
    ...
    utterance.rate = 0.92;        // Rusty: deliberate mechanic pace
    utterance.pitch = 0.75;        // Rusty: deep, authoritative garage voice
    utterance.volume = 0.95;
    
    // Rusty's signature voice (deep male)
    const voices = window.speechSynthesis.getVoices();
    const rustyVoice = voices.find(voice => 
      voice.name.includes('Brian') ||    // Deep male
      voice.name.includes('David') ||    // Deep male  
      voice.name.includes('Google US English')
    ) || voices.find(voice => voice.lang.startsWith('en-'));
    
    if (rustyVoice) {
      utterance.voice = rustyVoice;
    }
    
    window.speechSynthesis.speak(utterance);
  }}
  className="px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center gap-2"
>
  <span className="text-2xl">🔧</span>
  <span className="font-black">Rusty Speaks</span>
</button>
              </div>
            </div>
          </div>
